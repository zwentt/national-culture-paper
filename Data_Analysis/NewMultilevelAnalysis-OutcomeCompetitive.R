...
colnames(CultureData)[colnames(CultureData) == 'g1'] <- 'network4x'
CultureData$network4xcj <- CultureData$network4x - ave(CultureData$network4x, CultureData$country, FUN = function(x) mean(x, na.rm=T))
...
brm.outcome.model <-  outcome ~ mfr2013std + firmsize.adj.cj + (network2xcj:agility2cj) * cul + agilitycj * cul + 
                      (1 + network2xcj + agilitycj | countryx)
...
mcmcReg(list(outcome.guaiv.brm.fit,
        ...
        pars = c("b", "sd", "sigma"),
        ci = 0.95, format = "latex", caption = "Outcome Models - Cross Interaction 04-19",
        file = paste(filepath, "LatexTables/", "04192021-outcomeModel", sep=""), 
...
brm.outcome2.model <-  outcome2 ~ mfr2013std + firmsize.adj.cj + (network2xcj:agility2cj) * cul + agility2cj * cul + 
...
mcmcReg(list(outcome2.guaiv.brm.fit,
        ...
        pars = c("b", "sd", "sigma"),
        ci = 0.95, format = "latex", caption = "Outcome2 Models - Cross Interaction 04-19",
        file = paste(filepath, "LatexTables/", "04192021-outcome2Model", sep=""), 
        custom.model.names = c("UAI", "FUO", "PDI", "InsCol", "HUM", "PER", "IgrCol", "GND", "ASS"), regex = TRUE)
...
#7/6. Outcome2 Models - with four level firm network
rm(brm.outcome24x.fit.names)
brm.outcome24x.fit.names <- "outcome24x.control.brm.fit"

for (v in glo_v) {
  brm.outcome24x.fit.names <- c(brm.outcome24x.fit.names, paste("outcome24x.", v, ".brm.fit", sep = ""))
}

#Fitting Control Variable Only Model
outcome24x.control.brm.fit <- brm(outcome2 ~ mfr2013lnstd + network4xcj + firmsize.adj.cj + (1 | countryx), data = CultureData)

brm.outcome24x.model <- outcome2 ~ mfr2013lnstd + firmsize.adj.cj + I(network4xcj*network4xcj) * agility2cj * cul + (1 + network4xcj + agility2cj | countryx)
...
for (i in 1:9) {
  CultureData$cul <- as.numeric(unlist(CultureData[, c(glo_vc[i])]))
  assign(brm.outcome24x.fit.names[i+1], brm(brm.outcome24x.model, data = CultureData, core = 8, chains = 4, iter = 3000, refresh = 0, control = list(adapt_delta = 0.99)))
  #Give R a break to cool down the CPU
  Sys.sleep(20)
}
...
#Model Output using mcmcReg
mcmcReg(list(outcome24x.control.brm.fit,
             outcome24x.guaiv.brm.fit,
             outcome24x.gfuov.brm.fit,
             outcome24x.gpdiv.brm.fit,
             outcome24x.ginscolv.brm.fit,
             outcome24x.ghumv.brm.fit,
             outcome24x.gperv.brm.fit,
             outcome24x.gigrcolv.brm.fit,
             outcome24x.ggndv.brm.fit,
             outcome24x.gassv.brm.fit),
        pars = c("b", "sd", "sigma"),
        ci = 0.95, format = "latex", caption = "Outcome2 Models with 4 Level Network Squared Term - Cross Interaction",
        file = paste(filepath, "LatexTables/", "04152021-outcome24xsqModel", sep=""), 
        custom.model.names = c("Control", "UAI", "FUO", "PDI", "InsCol", "HUM", "PER", "IgrCol", "GND", "ASS"), regex = TRUE)
