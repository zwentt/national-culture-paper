...
fit.outcome2.null <- brm(outcome2.null, data = CultureData, refresh = 0, core = 7, family = "student")
...
agility2.control.brm.fit <- brm(agility2 ~ mfr2013.z + strategycj + firmsize.adj.cj + competitive1cj + (1 | countryx), data = CultureData, family = "student")
...
for (i in 1:9) {
  ...
  assign(brm.fit.names[i+1], brm(brm.agility2.model, data = CultureData, core = 8, chains = 4, iter = 3000, refresh = 0, control = list(adapt_delta = 0.99), family = "student"))
...
mcmcReg(list(agility2.guaiv.brm.fit,
        ...
        file = paste(filepath, "LatexTables/", "agility2CrossModelsCompetitive4T", sep=""), 
...
mcmcReg(list(agility2.guaiv.brm.fit,
        ...
        file = paste(filepath, "LatexTables/", "agility2CrossModelsCompetitiveT104", sep=""), 
        custom.model.names = c("UAI", "FUO", "PDI", "InsCol", "HUM", "PER", "IgrCol", "GND", "ASS"), regex = TRUE)
...
#Fitting Control Variable Only Model
agility2.control.brm.fit.regional <- brm(agility2 ~ mfr2013.z + strategycj + firmsize.adj.cj + competitive1cj + (1 | countryx), data = regionalData, family = "student")
...
for (i in 1:9) {
  ...
  assign(brm.fit.names[i+1], brm(brm.agility2.model.regional, data = regionalData, core = 8, chains = 4, iter = 3000, refresh = 0, family = "student", control = list(adapt_delta = 0.99)))
  #Give R a break to cool down the CPU
  Sys.sleep(15)
}
