...
outcome2.control.brm.fit <- brm(outcome2 ~ mfr2013.z + firmsize.adj.cj + strategycj + (1 | countryx), 
...
#model estimations 
for (i in 1:9) {
  brm.fit.file <- paste("./brmfit/", brm.outcome.fit.names[i+1], sep = "")
...
mcmcReg(list(outcome2.guaiv.brm.fit,
        ...
        pars = c("sd", "sigma", "b"),
        ci = 0.95, format = "latex", caption = "Outcome2 Models (Z) - 4 Level Network",
        file = paste(filepath, "LatexTables/", "outcome2FinalZ4", sep=""), 
...
mcmcReg(list(outcome2.guaiv.brm.fit,
             ...
             outcome2.gassv.brm.fit),
        pars = c("b", "sd", "sigma"),
        ci = 0.90, format = "latex", caption = "Outcome2 Models (Z) alpha = .10 - 4 Level Network",
        file = paste(filepath, "LatexTables/", "outcome2FinalZ410", sep=""), 
        custom.model.names = c("UAI", "FUO", "PDI", "InsCol", "HUM", "PER", "IgrCol", "GND", "ASS"), regex = TRUE)
...
for (m in 1:10) {
  ...
  z.model <- eval(parse(text = brm.outcome.fit.names[m]))
  
  fileName <- paste("./brmfit/", brm.outcome.fit.names.t[m], sep = "")
  assign(brm.outcome.fit.names.t[m], update(z.model, family = "student", cores = 8, chains = 4, 
                         iter = 3000, refresh = 0, control = list(adapt_delta = 0.99),
...
#Model Output using mcmcReg - 5% significance
mcmcReg(list(outcome2.guaiv.brm.fit.t,
             outcome2.gfuov.brm.fit.t,
             outcome2.gpdiv.brm.fit.t,
             outcome2.ginscolv.brm.fit.t,
             outcome2.ghumv.brm.fit.t,
             outcome2.gperv.brm.fit.t,
             outcome2.gigrcolv.brm.fit.t,
             outcome2.ggndv.brm.fit.t,
             outcome2.gassv.brm.fit.t),
        pars = c("sd", "sigma", "b"),
        ci = 0.95, format = "latex", caption = "Outcome2 Models (T) - 4 Level Network",
        file = paste(filepath, "LatexTables/", "outcome2FinalT4", sep=""), 
        custom.model.names = c("UAI", "FUO", "PDI", "InsCol", "HUM", "PER", "IgrCol", "GND", "ASS"), regex = TRUE)
...
mcmcReg(list(outcome2.guaiv.brm.fit.t,
        ...
        file = paste(filepath, "LatexTables/", "outcome2FinalT410", sep=""), 
        custom.model.names = c("UAI", "FUO", "PDI", "InsCol", "HUM", "PER", "IgrCol", "GND", "ASS"), regex = TRUE)
